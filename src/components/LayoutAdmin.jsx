...
const LayoutAdmin = () => {
  ...
  return (
    <div className="min-h-screen bg-gym-dark flex">
      ...
      {/* Sidebar - responsive */}
      <aside
        className={`fixed md:relative z-50 w-72 bg-gym-dark-secondary border-r border-gym-gray/10 h-full transition-transform duration-300 ease-in-out ${
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center justify-between h-16 px-6 border-b border-gym-gray/10">
            <span className="text-xl font-bold bg-gradient-to-r from-gym-neon to-gym-neon/70 bg-clip-text text-transparent">
              Sistema Gimnasio
            </span>
            {isMobile && (
              <button
          ...
          <nav className="flex-1 px-3 py-4 overflow-y-auto">
            {menuItems.map((item) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition-all duration-200 mb-1 ${
                  activeMenu === item.id
                    ? 'bg-gym-neon/10 text-gym-neon border border-gym-neon/20'
                    : 'text-gym-gray hover:bg-gym-card hover:text-gym-white'
                }`}
              >
          ...
          <div className="px-3 py-4 border-t border-gym-gray/10">
            <button
              onClick={handleLogout}
              className="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm text-gym-gray hover:bg-gym-danger/10 hover:text-gym-danger transition-all duration-200"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="flex-1 min-w-0">
        ...
        <header className="sticky top-0 z-30 bg-gym-dark-secondary/90 backdrop-blur-sm border-b border-gym-gray/10">
          <div className="flex items-center justify-between h-16 px-4 md:px-6">
            <div className="flex items-center gap-3">
              {/* Boton hamburguesa para móvil */}
              <button
                onClick={toggleSidebar}
                className="md:hidden text-gym-gray hover:text-gym-white transition-colors p-2 -ml-2"
              ...
              <h2 className="text-gym-white font-semibold text-lg truncate">
            ...
            <button
              onClick={goToProfile}
              className="flex items-center gap-2 text-gym-gray hover:text-gym-white transition-colors text-sm"
            >
